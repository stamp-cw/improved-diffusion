...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.c = c
        self.r = r
        self.t = np.linspace(1, len(betas), len(betas),dtype=np.int64)
        self.t_prev = np.pad(self.t[:-1], (1, 0),mode='constant', constant_values=0)
        ...
        self.p_alpha_zero = 0.
        self.p_alhpa_prev = np.pad(self.p_alhpa[:-1], (1, 0),mode='constant', constant_values=self.p_alpha_zero)
        self.p_beta = self.p_alhpa / self.t
        self.p_beta_zero = 1.
        self.p_beta_prev = np.pad(self.p_beta[:-1], (1, 0),mode='constant', constant_values=self.p_beta_zero)
        self.p_gamma = self.p_beta / self.p_beta_prev
        self.sqrt_p_gamma = np.sqrt(self.p_gamma)


        # x0 系数
        self.forward_noise_coef1 = np.sqrt(self.p_beta/self.p_beta_zero)
        # noise系数
        self.forward_noise_coef2 = self.c *  self.sqrt_p_alpha


        # x0 系数
        self.predict_start_from_noise_coef1 = np.sqrt(self.p_beta_zero / self.p_beta)
        # noise系数
        self.predict_start_from_noise_coef2 = self.c * np.sqrt(self.p_beta_zero * self.t)
